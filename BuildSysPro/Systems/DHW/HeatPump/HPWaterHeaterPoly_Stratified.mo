...
model HPWaterHeaterPoly_Stratified
  ...
  Modelica.Blocks.Interfaces.RealOutput T_tank annotation (Placement(
        transformation(extent={{100,-20},{140,20}}),iconTransformation(extent={{100,-20},
            {140,20}})));
  Components.StratifiedWaterTank stratifiedWaterTank(
  ...
  nCells=nCells,
  iHX_top=iHX_top,
  iHX_bottom=iHX_bottom,
  iAux_top=iAux_top,
  iAux_bottom=iAux_bottom,
  i_sensor=i_sensor,
  T_amb=T_amb,
...
protected
  ...
  Modelica.Blocks.Sources.RealExpression outletWaterTemperature(y=
        stratifiedWaterTank.T_tank[stratifiedWaterTank.nCells])
...
public
  Modelica.Blocks.Interfaces.RealOutput P_elec_hp "Heat pump electric power" annotation (Placement(
        transformation(extent={{100,110},{140,150}}), iconTransformation(extent={{100,40},
            {140,80}})));
  Modelica.Blocks.Interfaces.RealOutput COP_hp "Heat pump COP" annotation (Placement(
        transformation(extent={{100,70},{140,110}}), iconTransformation(extent={{100,100},
            {140,140}})));
equation
  P_elec_hp=waterHeaterHeatPump.Pe_tot;
  COP_hp=waterHeaterHeatPump.COP_tot;
  connect(drawOffScenario.drawoff_flow_rate,stratifiedWaterTank. m_flow)
    annotation (Line(points={{41.3,-49.9},{47.5556,-49.9},{47.5556,-2}}, color={
          0,0,127}));
  connect(T_in, drawOffScenario.T_cold) annotation (Line(points={{32,-100},{32,-70},
          {8,-70},{8,-44},{18,-44}}, color={0,0,127}));
  connect(T_in,stratifiedWaterTank. T_in) annotation (Line(points={{32,-100},{
          32,-70},{64.5556,-70},{64.5556,-2}},
                                            color={0,0,127}));
  connect(stratifiedWaterTank.T_sensor, T_tank) annotation (Line(points={{75.8889,
  ...
  connect(waterHeaterHeatPumpController.OnOff, waterHeaterHeatPump.OnOff)
    annotation (Line(points={{-27,39.0303},{-27,-7.3}},                   color=
         {255,0,255}));
  connect(waterHeaterHeatPumpController.OnOffAuxiliary,stratifiedWaterTank. onOffAuxiliary)
    annotation (Line(points={{-18.2,39.2242},{-18.2,14},{36.6,14}}, color={255,0,
          255}));
  ...
  connect(waterHeaterHeatPump.Pq_cond, stratifiedWaterTank.heatInput)
    annotation (Line(points={{-10.5,-14.2},{20,-14.2},{20,29.6},{36.2222,29.6}},
        color={0,0,127}));
  connect(T_ext_K.T, waterHeaterHeatPumpController.T_air) annotation (Line(
        ...
        color={0,0,127}));
  connect(stratifiedWaterTank.T_sensor, waterHeaterHeatPump.T_water)
    annotation (Line(points={{75.8889,27.6},{90,27.6},{90,80},{-66,80},{-66,
          -15.4},{-45,-15.4}},
                        color={0,0,127}));
  connect(stratifiedWaterTank.T_sensor, waterHeaterHeatPumpController.T_water)
    annotation (Line(points={{75.8889,27.6},{90,27.6},{90,80},{-66,80},{-66,
          67.5394},{-38.4,67.5394}},
                            color={0,0,127}));
...
<p>Air source heat pump water heater (ASHPWH) with mantle heat exchanger including both fixed and variable speed compressor, associated with stratified water tank.</p>
...
end HPWaterHeaterPoly_Stratified;
