...
parameter Boolean RegTOR = true "On/Off control ?";
  ...
  parameter Modelica.SIunits.TemperatureDifference THyst=1
...
public
 Modelica.Blocks.Interfaces.RealInput T2 "External temperature" annotation (
      Placement(transformation(
        ...
        rotation=180,
        origin={89,9})));
  Modelica.Blocks.Interfaces.RealOutput Y2 "PLR" annotation (Placement(
        transformation(
        extent={{-5,-5},{5,5}},
        rotation=270,
...
equation
  ...
  connect(ConditionChargeG.u,PLR. y) annotation (Line(
      ...
      color={0,0,127},
      smooth=Smooth.None));
  connect(LoiDEau.TconsK,PID_d. u_s) annotation (Line(
  ...
  connect(PID_d.y,PLR. u) annotation (Line(
      ...
      color={0,0,127},
      smooth=Smooth.None));
  connect(T2, LoiDEau.Text) annotation (Line(points={{99,19},{61.5,19},{61.5,18},
          {23,18}}, color={0,0,127}));
  connect(PLR.y, Y2)
...
<p>Generic model of water outlet temperature of a heating system based on a PID model. The main output is the part load ratio that should be connected to the system.</p>
