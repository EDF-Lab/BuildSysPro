...
model WaterDesignTemperature
  ...
  Modelica.Blocks.Interfaces.RealInput Text "Exterior temperature (°K)"
  ...
  Modelica.Blocks.Interfaces.RealOutput TconsK
    "Setpoint temperature (°K)"
    annotation (Placement(transformation(extent={{96,-10},{116,10}}),
        iconTransformation(extent={{96,-10},{116,10}})));
protected
  Modelica.Blocks.Tables.CombiTable1D LoiDEau(table=table)
...
equation
  if RetourDEau[1] > (TconsK + THyst) then
    u = false;
  elseif RetourDEau[1] < (TconsK - THyst) then
    u = true;
  else
    u = pre(u);
  end if;

  connect(Text, to_degC.u) annotation (Line(
      points={{-100,0},{-62,0}},
      color={0,0,127},
      smooth=Smooth.None));
  ...
  connect(LoiDEau.y[1], Tcons.u) annotation (Line(
      ...
      color={0,0,127},
      smooth=Smooth.None));
  connect(Tcons.y, TconsK) annotation (Line(
      points={{41,0},{106,0}},
      color={0,0,127},
      smooth=Smooth.None));
