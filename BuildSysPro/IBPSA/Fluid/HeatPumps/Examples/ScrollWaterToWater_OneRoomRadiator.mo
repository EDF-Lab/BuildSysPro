within BuildSysPro.IBPSA.Fluid.HeatPumps.Examples;
...
//--------------------------------------------------------------------------------------//
  ...
  IBPSA.Fluid.Sources.FixedBoundary sou(
    ...
    use_T=true,
    nPorts=1,
    T=281.15) "Fluid source on source side" annotation (Placement(
        transformation(extent={{-38,-208},{-18,-188}})));
  IBPSA.Fluid.Sources.FixedBoundary sin(
    redeclare package Medium = MediumW,
    use_T=true,
    nPorts=1,
    T=283.15) "Fluid sink on source side" annotation (Placement(
        transformation(extent={{88,-210},{68,-190}})));
  IBPSA.Fluid.Movers.FlowControlled_m_flow pumHeaPumSou(
    ...
    use_inputFilter=false,
    energyDynamics=Modelica.Fluid.Types.Dynamics.SteadyState)
    "Pump for heat pump source side" annotation (Placement(
        transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={2,-178})));
...
First implementation.
    ...
    __Dymola_Commands(file=
     "modelica://BuildSysPro/Resources/IBPSA/Scripts/Dymola/Fluid/HeatPumps/Examples/ScrollWaterToWater_OneRoomRadiator.mos"
        "Simulate and plot"),
    experiment(
      StopTime=172800,
      Tolerance=1e-08));
