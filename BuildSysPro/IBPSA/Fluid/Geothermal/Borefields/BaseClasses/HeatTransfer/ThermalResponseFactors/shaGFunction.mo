...
algorithm
  sha := IBPSA.Utilities.Cryptographics.sha(String(nBor, format=
    formatStrGen));
  sha := IBPSA.Utilities.Cryptographics.sha(sha + String(hBor,
    format=formatStrGen));
  sha := IBPSA.Utilities.Cryptographics.sha(sha + String(dBor,
    format=formatStrGen));
  sha := IBPSA.Utilities.Cryptographics.sha(sha + String(rBor,
    format=formatStrGen));
  sha := IBPSA.Utilities.Cryptographics.sha(sha + String(aSoi,
    format=formatStrGen));
  sha := IBPSA.Utilities.Cryptographics.sha(sha + String(nSeg,
    format=formatStrGen));
  sha := IBPSA.Utilities.Cryptographics.sha(sha + String(nTimSho,
    format=formatStrGen));
  sha := IBPSA.Utilities.Cryptographics.sha(sha + String(nTimLon,
    format=formatStrGen));
  sha := IBPSA.Utilities.Cryptographics.sha(sha + String(ttsMax,
    format=formatStrGen));
  for i in 1:nBor loop
    sha := IBPSA.Utilities.Cryptographics.sha(sha + String(cooBor[
      i, 1], format=formatStrCoo));
    sha := IBPSA.Utilities.Cryptographics.sha(sha + String(cooBor[
      i, 2], format=formatStrCoo));
  end for;
