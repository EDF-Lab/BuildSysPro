...
protected
  ...
  final parameter Boolean prePre=
    preVar ==IBPSA.Fluid.Movers.BaseClasses.Types.PrescribedVariable.PressureDifference  or
  ...
  final parameter Real etaDer[size(per.efficiency.V_flow,1)]=
    if not per.etaHydMet==IBPSA.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.Efficiency_VolumeFlowRate
    ...
    elseif (size(per.efficiency.V_flow, 1) == 1)
      ...
      IBPSA.Utilities.Math.Functions.splineDerivatives(
        ...
        ensureMonotonicity=IBPSA.Utilities.Math.Functions.isMonotonic(
  ...
  final parameter Real motDer[size(per.motorEfficiency.V_flow, 1)]=
    if not per.etaMotMet==IBPSA.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.Efficiency_VolumeFlowRate
  ...
  final parameter Real motDer_yMot[size(per.motorEfficiency_yMot.y,1)]=
    if not per.etaMotMet==IBPSA.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.Efficiency_MotorPartLoadRatio
  ...
  final parameter Real motDer_yMot_generic[9]=
    if per.etaMotMet==IBPSA.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.GenericCurve
      or  (per.etaMotMet==IBPSA.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.NotProvided
           and per.haveWMot_nominal)
      then IBPSA.Utilities.Math.Functions.splineDerivatives(
  ...
  parameter Real powDer[size(per.power.V_flow,1)]=
   if per.etaHydMet==IBPSA.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.Power_VolumeFlowRate
                                                                                          then
     IBPSA.Utilities.Math.Functions.splineDerivatives(
                   x=per.power.V_flow,
                   y=per.power.P,
                   ensureMonotonicity=IBPSA.Utilities.Math.Functions.isMonotonic(x=per.power.P,
                                                                                     strict=false))
   else
     zeros(size(per.power.V_flow,1))
    "Coefficients for polynomial of power vs. flow rate";

  final parameter IBPSA.Fluid.Movers.BaseClasses.Euler.powerWithDerivative powEu_internal=if (curve
       == 1) then IBPSA.Fluid.Movers.BaseClasses.Euler.power(peak=per.peak,
      pressure=pCur1) elseif (curve == 2) then
      IBPSA.Fluid.Movers.BaseClasses.Euler.power(peak=per.peak, pressure=pCur2)
       else IBPSA.Fluid.Movers.BaseClasses.Euler.power(peak=per.peak, pressure=
      pCur3) "Intermediate parameter";
  final parameter
    IBPSA.Fluid.Movers.BaseClasses.Characteristics.powerParameters powEu(V_flow=
        powEu_internal.V_flow, P=powEu_internal.P)
...
function getPerformanceDataAsString
  input IBPSA.Fluid.Movers.BaseClasses.Characteristics.flowParameters pressure
      "Performance data";
...
the simulation stops.");
  ...
  assert(not ((per.etaMotMet == IBPSA.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.Efficiency_MotorPartLoadRatio
     or per.etaMotMet == IBPSA.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.GenericCurve)
     and not per.haveWMot_nominal), "In " + getInstanceName() + ": etaMotMet is set to
...
equation
  ...
  if per.powerOrEfficiencyIsHydraulic then
    ...
    etaHyd = IBPSA.Utilities.Math.Functions.smoothMin(
      x1=eta/etaMot,
      x2=1,
      deltaX=1E-3);
  end if;
  ...
  if per.etaHydMet == IBPSA.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.Power_VolumeFlowRate then
    if homotopyInitialization then
    ...
    eta_internal = WFlo/IBPSA.Utilities.Math.Functions.smoothMax(
      ...
      x2=deltaP,
      deltaX=deltaP/2);
  elseif per.etaHydMet == IBPSA.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.EulerNumber then
    if homotopyInitialization then
  ...
  if per.etaMotMet == IBPSA.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.Efficiency_VolumeFlowRate then
    if homotopyInitialization then
  ...
  elseif per.etaMotMet == IBPSA.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.Efficiency_MotorPartLoadRatio then
    if homotopyInitialization then
  ...
  elseif per.etaMotMet == IBPSA.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.GenericCurve then
      if homotopyInitialization then
        etaMot =homotopy(actual=cha.efficiency_yMot(
...
<a href=\"modelica://BuildSysPro.IBPSA.Fluid.Movers.BaseClasses.PowerInterface\">
IBPSA.Fluid.Movers.BaseClasses.PowerInterface</a>.
</li>
<li>
