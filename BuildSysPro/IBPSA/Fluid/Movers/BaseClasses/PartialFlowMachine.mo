...
partial model PartialFlowMachine
  ...
  extends IBPSA.Fluid.Interfaces.LumpedVolumeDeclarations(final massDynamics=
        energyDynamics, final mSenFac=1);
  extends IBPSA.Fluid.Interfaces.PartialTwoPort(port_a(p(start=Medium.p_default),
        h_outflow(start=h_outflow_start)), port_b(p(start=p_start), h_outflow(
...
protected
  ...
  parameter IBPSA.Fluid.Movers.BaseClasses.Types.PrescribedVariable preVar
  ...
  Modelica.Blocks.Sources.Constant[size(stageInputs, 1)] stageValues(
    final k=stageInputs) if inputType == IBPSA.Fluid.Types.InputType.Stages
                                                      "Stage input values"
    annotation (Placement(transformation(extent={{-80,40},{-60,60}})));
  Modelica.Blocks.Sources.Constant setConst(
    final k=constInput) if inputType == IBPSA.Fluid.Types.InputType.Constant
  ...
  Modelica.Blocks.Math.Gain gaiSpe(y(final unit="1"))
    if inputType == IBPSA.Fluid.Types.InputType.Continuous and speedIsInput
  ...
  IBPSA.Fluid.Movers.BaseClasses.PowerInterface heaDis(final motorCooledByFluid=
       per.motorCooledByFluid, final delta_V_flow=1E-3*V_flow_max)
 ...
 if addPowerToMedium
  ...
  IBPSA.Fluid.Sensors.RelativePressure senRelPre(redeclare final package Medium =
        Medium) "Head of mover"
...
initial algorithm
  ...
  assert(
    nominalValuesDefineDefaultPressureCurve or (per.havePressureCurve or (
      preVar == IBPSA.Fluid.Movers.BaseClasses.Types.PrescribedVariable.Speed))
       or per.etaHydMet <> IBPSA.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.Power_VolumeFlowRate,
    "*** Warning: You are using a flow or pressure controlled mover with the
  ...
  assert(
    per.havePressureCurve or not (per.etaHydMet == IBPSA.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.Power_VolumeFlowRate
       or per.etaHydMet == IBPSA.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.EulerNumber),
    "*** Warning in " + getInstanceName() + ": per.etaHydMet is set to .Power_VolumeFlowRate or .EulerNumber.
  ...
  assert(
    per.havePressureCurve or per.haveWMot_nominal or not (per.etaMotMet ==
    ...
    "*** Warning in " + getInstanceName() + ": per.etaMotMet is set to .Efficiency_MotorPartLoadRatio
  ...
  assert(
    per.powerOrEfficiencyIsHydraulic or not (per.etaMotMet == IBPSA.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.Efficiency_MotorPartLoadRatio
       or per.etaMotMet == IBPSA.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.GenericCurve),
    "*** Warning in " + getInstanceName() + ": per.etaMotMet is set to .Efficiency_MotorPartLoadRatio
