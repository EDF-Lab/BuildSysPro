...
algorithm
  log_r_Eu := log10(IBPSA.Utilities.Math.Functions.smoothMax(
    x1=dp*peak.V_flow^2,
    x2=V_flow_dp_small,
    deltaX=V_flow_dp_small/2)/IBPSA.Utilities.Math.Functions.smoothMax(
    x1=peak.dp*V_flow^2,
    x2=V_flow_dp_small,
    deltaX=V_flow_dp_small/2));
  eta := peak.eta*IBPSA.Fluid.Movers.BaseClasses.Euler.correlation(x=log_r_Eu);
