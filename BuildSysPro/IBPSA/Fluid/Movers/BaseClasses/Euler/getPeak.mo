within BuildSysPro.IBPSA.Fluid.Movers.BaseClasses.Euler;
function getPeak
  ...
  output IBPSA.Fluid.Movers.BaseClasses.Euler.peak peak
    "Operation point at maximum efficiency";
  ...
  elseif max(power.P) < 1E-6 then
    ...
    if nPre == nPow then
      ...
      if nPre > nPow then
        ...
        dp:= pressure.dp;
        for i in 1:n loop
          P[i] := IBPSA.Utilities.Math.Functions.smoothInterpolation(
            x=V_flow[i],
            xSup=power.V_flow,
            ySup=power.P,
        ...
        P:= power.P;
        for i in 1:n loop
          dp[i] := IBPSA.Utilities.Math.Functions.smoothInterpolation(
            x=V_flow[i],
            xSup=pressure.V_flow,
            ySup=pressure.dp,
            ensureMonotonicity=true);
    ...
    eta:=V_flow.*dp./P;
    etaLes:=n<4;
    etaMon := IBPSA.Utilities.Math.Functions.isMonotonic(x=eta, strict=false);
    ...
    //   2. the available point with the highest efficiency.
      peak.eta := IBPSA.Utilities.Math.Functions.smoothInterpolation(
        ...
        xSup=V_flow,
        ySup=eta,
        ensureMonotonicity=IBPSA.Utilities.Math.Functions.isMonotonic(eta,
          strict=false));
...
<a href=\"modelica://BuildSysPro.IBPSA.Fluid.Movers.BaseClasses.Euler.peak\">
