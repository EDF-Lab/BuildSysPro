...
function power
  ...
  input IBPSA.Fluid.Movers.BaseClasses.Characteristics.flowParametersInternal pressure
    "Pressure curve with both max flow rate and max pressure";
  output IBPSA.Fluid.Movers.BaseClasses.Euler.powerWithDerivative power(
    V_flow=zeros(11),
    P=zeros(11),
    d=zeros(11)) "Power and its derivative vs. flow rate";
protected
...
algorithm
  ...
  for i in 2:10 loop
    power.P[i] := V_flow[i]*dp[i]/
      IBPSA.Fluid.Movers.BaseClasses.Euler.efficiency(
      peak=peak,
      dp=dp[i],
      V_flow=V_flow[i],
  ...
  power.d[2:10] := IBPSA.Utilities.Math.Functions.splineDerivatives(
    x=V_flow[2:10],
    y=power.P[2:10],
    ensureMonotonicity=false);
