within BuildSysPro.IBPSA.Fluid.Movers.Data;
record Generic "Generic data record for movers"
  ...
  parameter IBPSA.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod etaHydMet=IBPSA.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.
       EulerNumber
    "Efficiency computation method for the hydraulic efficiency etaHyd"
    annotation (Dialog(group="Power computation"));
  parameter IBPSA.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod etaMotMet=if
      powerOrEfficiencyIsHydraulic then IBPSA.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.GenericCurve
  ...
  parameter IBPSA.Fluid.Movers.BaseClasses.Characteristics.efficiencyParameters
    ...
    "Total or hydraulic efficiency vs. volumetric flow rate" annotation (Dialog(
        group="Power computation", enable=etaHydMet == IBPSA.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.Efficiency_VolumeFlowRate));
  parameter IBPSA.Fluid.Movers.BaseClasses.Characteristics.efficiencyParameters
    motorEfficiency(V_flow={0}, eta={0.7})
    "Motor efficiency vs. volumetric flow rate" annotation (Dialog(group=
          "Power computation", enable=etaMotMet == IBPSA.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.Efficiency_VolumeFlowRate));
  parameter
    IBPSA.Fluid.Movers.BaseClasses.Characteristics.efficiencyParameters_yMot motorEfficiency_yMot(y={0},
      eta={0.7})
    "Motor efficiency vs. part load ratio yMot, where yMot = WHyd/WMot_nominal"
    annotation (Dialog(group="Power computation", enable=etaMotMet == IBPSA.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.Efficiency_MotorPartLoadRatio));

  // Power curve
  //   It requires default values to suppress Dymola message
  //   "Failed to expand the variable Power.V_flow"
  parameter IBPSA.Fluid.Movers.BaseClasses.Characteristics.powerParameters power(V_flow={0},
      P={0}) "Power (either consumed or hydraulic) vs. volumetric flow rate"
    annotation (Dialog(group="Power computation", enable=etaHydMet == IBPSA.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.Power_VolumeFlowRate));
  ...
  parameter IBPSA.Fluid.Movers.BaseClasses.Euler.peak peak(
    ...
    annotation (Dialog(group="Power computation", enable=etaHydMet == IBPSA.Fluid.Movers.BaseClasses.Types.HydraulicEfficiencyMethod.EulerNumber));
  final parameter IBPSA.Fluid.Movers.BaseClasses.Euler.peak peak_internal=
      IBPSA.Fluid.Movers.BaseClasses.Euler.getPeak(pressure=pressure, power=
      power) "Internal peak variable";
  ...
  parameter Modelica.Units.SI.Power WMot_nominal(final displayUnit="W")=
    ...
    "Rated motor power"
      annotation(Dialog(group="Power computation",
                        enable=etaMotMet == IBPSA.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.Efficiency_MotorPartLoadRatio
           or etaMotMet == IBPSA.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.GenericCurve));
  parameter Modelica.Units.SI.Efficiency etaMot_max(max=1)= 0.7
    "Maximum motor efficiency"
    annotation (Dialog(group="Power computation", enable=etaMotMet == IBPSA.Fluid.Movers.BaseClasses.Types.MotorEfficiencyMethod.GenericCurve));
  final parameter
    IBPSA.Fluid.Movers.BaseClasses.Characteristics.efficiencyParameters_yMot motorEfficiency_yMot_generic=
      IBPSA.Fluid.Movers.BaseClasses.Characteristics.motorEfficiencyCurve(
      P_nominal=WMot_nominal, eta_max=etaMot_max)
    "Motor efficiency  vs. part load ratio" annotation (Dialog(enable=false));
