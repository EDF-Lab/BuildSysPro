...
model Orifice "Model with an orifice"
  ...
  IBPSA.Airflow.Multizone.Orifice ori(redeclare package Medium = Medium, A=0.2)
    "Orifice" annotation (Placement(transformation(extent={{0,20},{20,40}})));
  ...
  IBPSA.Fluid.Sources.Boundary_pT roo2(
    ...
    use_p_in=true,
    nPorts=1,
    T=293.15) "Pressure boundary condition" annotation (Placement(
        transformation(
        origin={70,30},
        extent={{-10,-10},{10,10}},
        rotation=180)));
...
experiment(Tolerance=1e-06, StopTime=1),
    __Dymola_Commands(file="modelica://BuildSysPro/IBPSA/Resources/Scripts/Dymola/Airflow/Multizone/Examples/Orifice.mos"
        "Simulate and plot"),
