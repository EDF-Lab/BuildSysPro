within BuildSysPro.IBPSA.Utilities.Psychrometrics.Functions.BaseClasses.Examples;
...
equation
  ...
  y = IBPSA.Utilities.Psychrometrics.Functions.pW_TDewPoi(T=T);
  der(y)=der(y_comp);
  err = y-y_comp;
  assert(abs(err)/max(1, abs(y)) < 1E-2, "Derivative implementation has an error or solver tolerance is too low.");
annotation (
  __Dymola_Commands(file="modelica://BuildSysPro/Resources/IBPSA/Scripts/Dymola/Utilities/Psychrometrics/Functions/BaseClasses/Examples/DewPointTemperatureDerivativeCheck.mos"
        "Simulate and plot"),
      experiment(
        StartTime=-1,
        StopTime=1,
