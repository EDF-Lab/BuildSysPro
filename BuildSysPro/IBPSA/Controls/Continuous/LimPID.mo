within BuildSysPro.IBPSA.Controls.Continuous;
block LimPID
  ...
  parameter Real y_reset=xi_start
    "Value to which the controller output is reset if the boolean trigger has a rising edge, used if reset == IBPSA.Types.Reset.Parameter"
    annotation(Dialog(enable=reset == IBPSA.Types.Reset.Parameter,
  ...
  Utilities.Math.IntegratorWithReset I(
    final reset=if reset ==IBPSA.Types.Reset.Disabled  then reset else IBPSA.Types.Reset.Input,
...
protected
  ...
  Modelica.Blocks.Sources.RealExpression intRes(
    final y=y_reset_internal/k - addPID.u1 - addPID.u2) if reset <> IBPSA.Types.Reset.Disabled
...
<ul>
  <li>
  If <code>reset = IBPSA.Types.Reset.Disabled</code>, which is the default,
  then the controller output is never reset.
  </li>
  <li>
  If <code>reset = IBPSA.Types.Reset.Parameter</code>, then a boolean
